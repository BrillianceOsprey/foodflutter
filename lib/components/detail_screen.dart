...
class _DetailScreenState extends State<DetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
           ...
           Navigator.push(context, MaterialPageRoute(builder: (context)=> CartView()));
            ...
            children: [
             //  SvgPicture.asset("assets/icons/basket.svg", color: Colors.white, ),
              Text("${cart.getLength()}", style: const TextStyle(color: Colors.white,fontWeight: FontWeight.bold),)
            ],
          ))
        ],
      ),
      ...
      body: Column(
        children: [
          Image.network(widget.product.image,height: MediaQuery.of(context).size.height*0.3,
       ...
       Expanded(child: SingleChildScrollView(
        child: Container(
          height: MediaQuery.of(context).size.height*0.7, // 1 full screen 0.7 and 0.3
          ...
          child: Column(
            children: [
              Row(
                ...
                child: Row(
                  ...
                  Column(
                    children: [
                      Text(widget.product.name, style: const TextStyle(color: Colors.white, fontSize: 20),)
                    ],
                  ),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text("${widget.product.price}", style: const TextStyle(color: Colors.blue),),
                        const Text("MKK", style: TextStyle(color: Colors.blue),)
                      ],
                    ),
                  )
                ],),
            ...
            const  SizedBox(height: 20,),
              Text(widget.product.description, style: const TextStyle(color: Colors.white),),
             const SizedBox(height: 20,),
              Row(
                ...
                children: [
                 ...
                 cart.has(CartProduct(this.counter, widget.product))? // terminal operator //check true of false
              ...
              //  Text("${cart.getCount(CartProduct(this.counter, widget.product))}"):
                Text("${updateCounter+ cart.getCount(CartProduct(this.counter, widget.product))}", style: TextStyle(color: Colors.white, fontSize: 20),):
                 //3 ("${counter}") , if false, this will work
                  Text("${counter}", style: const TextStyle(color: Colors.white, fontSize: 20),),
                  IconButton(onPressed: (){
                   setState(() {
                    //  counter++;
                      this.counter++;
                  ...
                  child: Row(
                   ...
                   mainAxisAlignment: MainAxisAlignment.center,
                    ...
                    // SvgPicture.asset("assets/icon/basket.svg"),
                     ...
                     Container(width: 20,),
                     const Text("Order Now", style: TextStyle(color: Colors.pink, fontWeight: FontWeight.bold, fontSize: 20),)
                    ],
                  ),
                  ),
                ),
